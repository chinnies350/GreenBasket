...
class CatagoryList extends Component {
  ...
  actionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    let links = [];
    links.push(
      <InoIcons.IoMdTrash title="Delete" onClick={() => this.deleteFun(row)} />
    );
  ...
  deleteFun = async (row) => {
    ...
    if (response.data.statusCode !== 1)
      return await this.addNotification(response.data.message, "warning");
    if (response.data.statusCode === 1) {
