...
class UserDashboard extends PureComponent {
  ...
  cartloginCheck = async (event, data) => {
    ...
    if (!checked) return;
    if (await this.getUserInfo()) return await this.addtoCart(data);
  ...
  wishlistloginCheck = async (event, data) => {
    ...
    if (!checked) return;
    if (await this.getUserInfo()) return await this.addtoWishlist(data);
    return this.props.props.history.push(`${subDirectory}/auth/identifier`);
  };
  ...
  render() {
    ...
    let breadCrumbItems = {
      title: pageName,
      items: [
        { name: "Home", active: false, link: `${subDirectory}/user/home` },
                      ...
                      <div className="col-md-12">
                        <div className="d-flex flex-row heart-cover-div">
                          <div className="cart-div">
                            <input
                              id={`toggle-cart${i}`}
                              onClick={(e) => this.cartloginCheck(e, c)}
                          ...
                          <div className="heart-div">
                            <input
                              id={`toggle-heart${i}`}
                              onClick={(e) => this.wishlistloginCheck(e, c)}
