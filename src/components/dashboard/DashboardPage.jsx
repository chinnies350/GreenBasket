...
class Dashboard extends PureComponent {
  ...
  getOrederDetails = async () => {
    ...
    if (!statusCode) return this.setState({ data: [] });
    let completedProducts = await _.filter(
      data,
      (v) => v["deliveryStatus"] === "Y"
    );
    let inProgressProducts = await _.filter(
      data,
      (v) => v["deliveryStatus"] === "N"
    );
    await this.setState({
              ...
              {categoryName &&
                categoryName.map((c, i) => (
                  <DashBox
                    ...
                    path={`${subDirectory}/dashboard/product-details`}
          ...
          {deliveryCounts && (
            <Col md={4} sm={12}>
              <DashBox
                ...
                icon={<IonIcons.IoMdDoneAll />}
                props={this.props}
                name={"product-status"}
                path={`${subDirectory}/dashboard/product-status`}
                data={completedProducts}
                cname={"Order Delivered"}
              />
            </Col>
          )}
          {deliveryCounts && (
            <Col md={4} sm={12}>
              <DashBox
                ...
                topic="Pending"
                value={pendingCount}
                icon={<IonIcons.IoIosTime />}
                props={this.props}
                name={"product-status"}
                path={`${subDirectory}/dashboard/product-status`}
                data={inProgressProducts}
                cname={"Pending Orders"}
              />
            </Col>
          )}
        </Row>
