...
import { getCookieData ,storeCookieData } from "../components/common/CookiesFunc"
class ForgotPassword extends PureComponent {
  ...
  onSubmit = async () => {
    ...
    if (data.otp) {
      if (data.otp !== this.state.userOtp) {
        this.addNotification("OTP mismatch", "error");
    ...
    if (data.passWord !== data.CpassWord) {
      ...
      if (res.data.statusCode === 1) {
        ...
        await this.props.props.history.push(`${subDirectory}/auth/identifier`)
      }
    }
  ...
  addNotification = (message, variant ) => {
    ...
    const options = { variant, anchorOrigin: { vertical: "bottom", horizontal: "center", autoHideDuration: 500 } };
    enqueueSnackbar(message, options)
  }

  render() {
    const { otpFeild, passwordFeild } = this.state
    let logo = getCookieData("logo")
    let loginimage = getCookieData("signImage")
        ...
        <Container>
          <Row>
            <Col md={5}>
              ...
              <img src={logo} alt="LoginIMage" style={{ width: "40%",marginLeft:"33%"}} />
            ...
            <Col md={7} style={{ textAlign: 'center' }}>
              <img src={loginimage} alt="LoginIMage" style={{ width: '68%', height: '394px' }} />
